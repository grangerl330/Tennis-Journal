...
class OpponentWeaknessesModal extends Component {
  constructor(props){
    super(props)
  ...
  componentDidMount(){
    this.setState({
      ...
      numberOfWeaknessInputs: this.props.opponent.weaknesses.length
    })

    this.props.opponent.weaknesses.forEach((weakness, index) => {
      this.setState({
  ...
  handleOnSubmit = event => {
    ...
    for(let i=0; i < this.state.numberOfWeaknessInputs; i++) {
      if(!opponent.weaknesses.includes(this.state[`weakness${i+1}`]) && this.state[`weakness${i+1}`] !== undefined) {
        opponent.weaknesses.push(this.state[`weakness${i+1}`])
      }
    }
  ...
  render(){
    return(
      <div className="modal fade" id="opponentWeaknessesModal">
...
export default withRouter(OpponentWeaknessesModal)
